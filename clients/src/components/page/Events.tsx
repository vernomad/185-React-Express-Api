...
if (error) return <p>Error loading events: {error.message}</p>;
  return (
    <div className='main-grid bg' id='main-grid-events'>
      <div className='events-wrapper'>
        <div id='3d-events' className="container-3d">

       <div id='grid-lines-events-ceiling' className="grid-lines ceiling"></div>
       <div id='grid-lines-events-floor' className="grid-lines floor"></div>
        ...
        <div className='calender-wrapper'>
          <Calendar
            ...
            onSelectEvent={(event) => setWallEvent(event as CalendarEventEntry)}
            selectable   // 👈 enables slot selection
                onSelectSlot={handleSelectSlot}  // 👈 callback for creating new events
              
          />
          <div className='plate-container'>
            <img src='assets/plate.png' style={{width: "100px", minWidth: "90px", paddingRight: "1rem"}} alt='' />
        </div>
        </div>
 ...
 <span>Select for view on wall</span>
        ...
        <div className='event-wall' key={wallEvent.id}>
           <div>
        <img src={wallEvent.image?.full} className='wall-img' style={{width: "80px", border: "2px solid gray"}} alt='' />
      </div>
      <div> <h3><strong>{wallEvent.title}</strong></h3></div>
      <div><p>Desc: {wallEvent.description}</p></div>
      <div><p>Location: {wallEvent.location}</p></div>
      <div><p>Starts: {new Intl.DateTimeFormat('en-US', {
              hour: 'numeric',
              minute: '2-digit',
              hour12: true
            }).format(wallEvent.start)}</p></div>
        </div>
